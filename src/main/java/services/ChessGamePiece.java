...
import interfaz.ChessGraveyard;
 ...
 * Classes to extend this piece are piesas.Rook, piesas.Bishop, piesas.Knight, piesas.Queen, piesas.King and piesas.Pawn.
...
public abstract class ChessGamePiece implements Serializable {
     ...
     * player's piesas.King to be put in check (which is an illegal move).
     *
     * @param board
     ...
     * in GamePiece and not piesas.ChessGameBoard because different pieces have
    ...
    public boolean isEnemy(ChessGameBoard board, int row, int col ){
        ...
        if ( this.getColorOfPiece() == ChessGamePiece.BLACK && enemyPiece.getColorOfPiece() == ChessGamePiece.WHITE ){
            isEnemy = true;
        }
        return isEnemy;
    }
